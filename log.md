...
**可以发现从数据结构的建立到查询同样多的数据，AVL树的查询性能要更好一些。且比哈希表将近快了一倍。**



后来我想，会不会是因为哈希表的建立太浪费时间了，如果我们撇开AVL树和哈希表的建立不看，只看这两种数据结构的查询时间的话，我们再来测一次。这次就要修改start的位置。

<img src="https://cdn.jsdelivr.net/gh/mLittle-horse/PicStore/img/20210314233728.png" alt="image-20210314233726936" style="zoom:67%;" />



<img src="https://cdn.jsdelivr.net/gh/mLittle-horse/PicStore/img/20210314233414.png" alt="image-20210314233412807" style="zoom:67%;" />



这一次可以发现，**如果不算建立这个数据结构的时间的话，哈希表其实要比AVL数略快。**

哈希表最花时间的地方其实就是建立哈希表。

但是这里其实不太能看出来，因为数据量不够大，我统计出不同用户名的数量之后输出来看了一下，这10万个数据里面一共有1150个不同的用户，也就是说我们刚刚测试这两种数据结构其实数据量只有1150个，并不能说明什么问题。

我觉得当数据量再大一点的话，只看查询速度，哈希表的总用时应该是比二分查找要小的。

但是当数据量太大的时候，建立哈希表又有些得不偿失。
